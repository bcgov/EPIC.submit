...
export default function ActionButtons() {
  ...
  const handleSaveAndClose = async () => {
    ...
    try {
      const decisionData = iemReviewSchema.validateSyncAt(validateAtKey, data);
      const passed = decisionData.passedReview === RadioOptions.YES.value;
      const updateRequestData = passed
        ? {}
        : iemReviewSchema.validateSyncAt("update_request", data);
      const requestBody = {
        form_answers: {
          ...decisionData,
          ...updateRequestData,
  ...
  const handleSendToManager = async () => {
    try {
      ...
      const staffDecision = iemReviewSchema.validateSyncAt(
        "staff",
        getValues(),
      );
      ...
      const updateRequestData = passed
        ? {}
        : iemReviewSchema.validateSyncAt("update_request", getValues());
  ...
  const handleCompletingReview = async () => {
    try {
      ...
      const managerDecision = iemReviewSchema.validateSyncAt(
        "manager",
        getValues(),
      );
