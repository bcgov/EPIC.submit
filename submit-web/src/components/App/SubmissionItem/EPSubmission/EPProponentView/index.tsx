...
import {
  EngagementPlanSubmissionForm,
  engagementPlanSubmissionSchema,
} from "./constants";
...
export const EngagementPlanProponentView = () => {
  const {
  ...
  const defaultDocumentValues = useMemo(() => {
    ...
    return {
      engagementPlan: documentSubmissions
        .filter(
          (submission) =>
            submission.submitted_document?.folder ===
            S3_FOLDER.ENGAGEMENT_PLANS.value,
        )
        .map((submission) => submission.submitted_document?.url),
      supportingEngagementPlan: documentSubmissions
        .filter(
          (submission) =>
            submission.submitted_document?.folder ===
            S3_FOLDER.ENGAGEMENT_PLAN_SUPPORTING_DOCUMENTS.value,
        )
        .map((submission) => submission.submitted_document?.url),
    };
  }, [documentSubmissions]);

  const methods = useForm<EngagementPlanSubmissionForm>({
    resolver: yupResolver(engagementPlanSubmissionSchema),
    mode: "onSubmit",
    defaultValues: {
    ...
    try {
      ...
      await callSaveSubmission({
        data: {
          ...
          data: {}, // No form fields for Engagement Plan
        },
      });
      await refetch();
