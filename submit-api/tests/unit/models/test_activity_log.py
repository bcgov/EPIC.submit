...
def test_get_activity_logs_for_staff(session):
    log = ActivityLog(
        entity_type='PACKAGE', entity_id=1, action='SUBMITTED',
        actor_id='user-1', actor_type='STAFF', visibility='staff'
    )
    session.add(log)
    session.flush()
...
def test_get_activity_logs_public_only(session):
    session.add(ActivityLog(
        entity_type='PACKAGE', entity_id=2, action='SUBMITTED',
        actor_id='user-1', actor_type='STAFF', visibility='staff'
    ))
    session.add(ActivityLog(
        entity_type='PACKAGE', entity_id=2, action='APPROVED',
        actor_id='user-1', actor_type='STAFF', visibility='public'
    ))
    session.flush()
...
def test_to_dict(session):
    log = ActivityLog(
        entity_type='PACKAGE', entity_id=3, action='SUBMITTED',
        actor_id='user-1', actor_type='STAFF', visibility='staff',
