"""Test StaffUser model ORM methods."""
...
def test_get_by_guid(session):
    """Returns the staff user associated with the given auth guid."""
...
def test_create_staff_user(session):
    """Creates a staff user with the correct name fields including computed full_name."""
    user = factory_user_model(user_type=UserType.STAFF)
    staff = StaffUser.create_staff_user(
        {
            'first_name': 'Alice',
